...
Both files sit in `doc/ws-<name>/` (same folder — the pair is inseparable). Verify coverage against the **brief**: every screen + every display/outcome state + every flow step has a matching `tabs` / `states` / `html` entry. **If `appendix/mockups.md` exists, also cross-check against it** — anything the ASCII enumerated that the brief-driven render lacks is a gap: add it (and note the brief should have listed it). Confirm the data file parses (no unescaped backticks inside template strings). Tell the user which design system was used (you must have one — if none resolved, you should have STOPPED at the input step). Suggest they double-click `mockups.html` to review.

### 5. Finalize `00-START-HERE.md`
This is the pipeline's terminal step. `00-START-HERE.md` already exists (written by `run`, updated by `agent-domain-spec` and `grill-to-brief`) — open it and UPDATE it, don't create a new file. Fill the sections only this stage can complete: confirm the prototype is ready (point at `mockups.html` + `mockups.data.js`, the inseparable pair), the **Scope** section (what the pack does/doesn't cover — no backend, no FE↔BE contract, WTP still an unverified assumption), **Bước kế** per receiver (non-tech review · interactive-prototype upload · Phase-2 FE build), and **Design system đã dùng** (the exact tokens/components file, so a later step can reproduce the skin). Tick `design-a-screen` + `brief-to-html` in the pipeline-status checklist and refresh the "cập nhật lần cuối" line. This turns the whole workspace into a handoff a stakeholder / web-app / Phase-2 dev can pick up without asking — there is no separate `HANDOFF.md`; `00-START-HERE.md` is the one file that has carried this job since the very first stage.
