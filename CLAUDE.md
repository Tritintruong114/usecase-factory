...
code or an FE↔BE contract. That's Phase-2 FE, a separate repo.
...
`skills/grill-to-brief/templates/`. If you change a template's headings, **update the matching
validator** — heading names are a contract (see below).
...
## Agents
`agents/*.md` are the spawnable workers (4 research workers A–D + 1 adversarial
decision-gate-reviewer). They're auto-discovered from `agents/`. `run`'s playbook fans them out
in **one message with multiple Agent calls** so they run concurrently. Read-only by design.
...
# Confirm skills + all 5 agents are discovered:
...
# or: npm run validate:dossier / npm run validate:coverage
```
